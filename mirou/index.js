...
class Dpdf {
  ...
  async getNextSentence(
    ...
    for (const connectorId of connectorIds) {
      ...
      const sentenceId =
        connector.start.item === id ? connector.end.item : connector.start.item;

      if (!sentenceId) {
        throw new Error("I did not find a fitting sentence id. Maybe a loop?");
      }
  ...
  async syncParagraphsOfChapter(num) {
    ...
    for (const firstConnectorId of chapter.connectorIds) {
      ...
      let curve = await this.getNextSentence(chapter.id, nextConnectorIds, [
        "curve",
      ]);
      ...
      while (true) {
        ...
        nextConnectorIds.push(
          ...curve.connectorIds.filter((id) => !nextConnectorIds.includes(id))
        );
        ...
        const circle = await this.getNextSentence(curve.id, nextConnectorIds, [
          "circle",
        ]);

        if (!circle) {
          throw new Error(
            `Didn't find a circle sentence for curve ${JSON.stringify(curve)}.`
          );
        }
    ...
    const lastSentenceOfChapter = (
      await miro.board.get({
        id: lastParagraphOfChapter[lastParagraphOfChapter.length - 1].id,
      })
    )[0];
